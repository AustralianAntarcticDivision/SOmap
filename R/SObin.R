...
SObin <- function(x, y = NULL,baselayer=NULL, ..., col = viridisLite::viridis(26), dim = c(300, 300), add = TRUE, target = NULL, source = NULL, data.frame=FALSE) {

  if (dev.cur() == 1L && isTRUE(add)) {
  ...
  if (missing(baselayer) && isFALSE(add)) {
