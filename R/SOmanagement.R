...
SOmanagement_inner <- function(ccamlr, ccamlr_labels, ssru, ssru_labels, ssmu, ssmu_labels, rb, rb_labels, sprfmorb, trim, eez, eez_labels, mpa, mpa_labels, iwc, iwc_labels, domains, domains_labels, rb_col, sprfmo_col, ccamlr_col, ssru_col, ssmu_col, eez_col, mpa_col, iwc_col, domains_col, basemap) {
    ...
    if (iwc) {
        ...
        temp <- c(if (trim > -78.40) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(-170, trim), c(-170, -78.40)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -73.844137) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(-120, trim), c(-120, -73.844137)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -75.146206) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(-60, min(trim, -65.168)), c(-60, -75.146206)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -62.4505) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(-60, trim), c(-60, -62.4505)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -69.596701) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(0, trim), c(0, -69.596701)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -68.366691) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(70, trim), c(70, -68.366691)), out$projection, source = "OGC:CRS84"), col = iwc_col)),
                  if (trim > -66.295027) SO_plotter(plotfun = "lines", plotargs = list(x = reproj::reproj_xy(rbind(c(130, trim), c(130, -66.295027)), out$projection, source = "OGC:CRS84"), col = iwc_col)))
        if (length(temp) > 0) {
            out$iwc <- temp
            if (iwc_labels) {
