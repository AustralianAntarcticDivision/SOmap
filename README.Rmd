...
An **automatic** plot function `SOmap_aut()` will take any data in the form of longitude and latitude vectors and create a guess at a map.

```{r automap, fig.height = 5, fig.width = 5}
ellie <- SOmap_data$mirounga_leonina

## construct and plot the map
SOmap_aut(ellie$lon, ellie$lat)
```

The `SOmap_aut`, `SOmap`, and `SOmap2` functions return the data used to make the map so that further customization can be made. Plotting or printing the returned object will cause the map to be displayed in the graphics device.
...
alb_map <- SOmap_aut(albatrack[, 1], albatrack[, 2])
...
SOmap_aut(mydata, target = "laea", centre_lon = 147, input_points = FALSE, lcol = 2)
...
```{r auto-setup}
set.seed(1)
amap <- SOmap_aut(input_points = FALSE, input_lines = FALSE)
amap
```
